...
class IntegrationBuilder {
  ...
  Future<void> makeProject() async {
    ...
    final result = Process.runSync('chmod', ['755','bin/protoc-gen-localproject'],workingDirectory: projectPath);
  ...
  Future<void> runBufExports() async {
    ...
    for (final bufModule in ['googleapis/googleapis', 'bestowcode/flakka']) {
      final result = Process.runSync('buf', ['export', 'buf.build/$bufModule', '-o', 'proto_vendor'],
